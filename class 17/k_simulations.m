%function avg = acc(N)
N = 500;
%scale = [0.00001 0.00005 0.0001 0.0005 0.001];
...
for snr = 1:35
    ...
    accuracy_y25 = 0;
    for k = 1:100
        data = data_generation_25classes_cfo(N,snr,cfo,scale(e));
        sig1 = data(:,1);
        sig7 = data(:,7);
        sig13 = data(:,13);
        sig19 = data(:,19);
...
title(strcat('CFO = 0.0001, Average Accuracy N = ',num2str(N)))
xlabel('SNR')
ylabel('Accuracy(in %)')
hold on
grid on
end
legend({'k = 1.2','k = 1.4','k = 1.6','k = 1.8','k = 2.0', 'k = 2.2'},'Location','southeast')
figure(2)
for i =1:length(scale)
plot(sigma,acc_1(i,:),'-o')
title(strcat('CFO = 0.0001, BPSK-BPSK N = ',num2str(N)))
xlabel('SNR')
ylabel('Accuracy(in %)')
hold on
grid on
...
title(strcat('CFO = 0.0001, QPSK-QPSK N = ',num2str(N)))
xlabel('SNR')
ylabel('Accuracy(in %)')
hold on
grid on
...
title(strcat('CFO = 0.0001, 8PSK-8PSK N = ',num2str(N)))
xlabel('SNR')
ylabel('Accuracy(in %)')
hold on
grid on
...
title(strcat('CFO = 0.0001, 8QAM-8QAM N = ',num2str(N)))
xlabel('SNR')
ylabel('Accuracy(in %)')
hold on
grid on
...
title(strcat('CFO = 0.0001, 16QAM-16QAM N = ',num2str(N)))
xlabel('SNR')
ylabel('Accuracy(in %)')
hold on
grid on
