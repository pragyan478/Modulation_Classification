...
if shift
    rxSig = rxSig.*exp(-1j*2*pi*(f_shift/RX_sample_rate).*(0:length(rxSig)-1)); % shift to DC
end
...
dfo=fo(2)-fo(1);
[diff, best_est] = cumulant_warped_feat(rxSig2(1:1e5), snr_est, RX_SPS, 20, Beta1_est);
...
disp(" ")
disp(" ")
total_power=mean(abs(rxSig2.^2));
R=1/(1+(10.^(0.1*pow_diff)));%(power_diff=R/1-R)

h2=sqrt(R*total_power);
h1=sqrt((1-R)*total_power);
ph1=angle(mean(rxSig2(1:1e5).^2))/2;%Stronger User
ph2=angle(mean(exp(-1j*2*pi*dfo*(1:1e5)).*rxSig2(1:1e5).^2))/2;
%%%%%% Works till here ^^ %%%%%%%%%%
